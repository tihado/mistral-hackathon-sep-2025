...
def get_s3_client():
    ...
    if not config['aws_access_key_id'] or not config['aws_secret_access_key']:
        raise NoCredentialsError("AWS credentials not found. Please set AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY environment variables.")
...
def generate_image_with_openrouter(api_key: str, prompt: str, image_urls: list) -> Optional[str]:
...
def virtual_try_on(product_image_data: str, user_image_data: str = None, category: Literal["clothing", "furniture", "other"] = "clothing") -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if category == "furniture":
            prompt = create_furniture_try_on_prompt("furniture item")
        elif category == "other":
            prompt = create_other_try_on_prompt("other item")
        else:
            prompt = create_clothing_try_on_prompt("clothing item")
        
        # Update prompt to include user reference if user image is provided
        if user_image_data and len(image_urls) > 1:
            if category == "clothing":
                prompt = f"""Create a realistic image showing this person wearing the clothing item from the first image. 
                The person should be wearing the clothing item naturally and comfortably. 
                The image should be high quality, well-lit, and show the clothing fitting properly on the person. 
                Make sure the clothing looks natural and realistic on the person."""
            elif category == "furniture":
                prompt = f"""Create a realistic image showing this person in a room with the furniture item from the first image. 
                The furniture should be positioned naturally in the space and complement the room's decor. 
                The image should be high quality, well-lit, and show the furniture fitting well in the room environment. 
                Make sure the furniture looks natural and realistic in the room setting."""
            else:
                prompt = f"""Create a realistic image showing this {category} item from the first image. 
                The {category} should be positioned naturally in the space and complement the room's decor. 
                The image should be high quality, well-lit, and show the {category} fitting well in the room environment. 
                Make sure the {category} looks natural and realistic in the room setting."""
        
        # Generate image using OpenRouter with URLs
        try:
            generated_image_data = generate_image_with_openrouter(api_key, prompt, image_urls)
